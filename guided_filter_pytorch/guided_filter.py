...
class GuidedFilter2d(nn.Module):
    def __init__(self, radius: int, eps: float, color_seperate: bool):
        super().__init__()
        self.r = radius
        self.eps = eps
...
def guidedfilter2d_color(guide, src, radius, eps, scale=None, return_ab=False):
    ...
    # a_test = torch.einsum("bijhw,bjchw->bichw", (inv_sigma, cov_Ip)) # b x 3 x C x H x W
    # # Check if a_test is equal to a
    # assert torch.allclose(a, a_test, atol=1e-6), "a_test is not equal to a"

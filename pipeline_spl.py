...
def ddcm_sampler(scheduler, x_s, x_t, timestep, e_s, e_t, x_0, noise, eta, to_next=True,
                 loss_fn=None, source_image=None, vae=None):
    ...
    # Image optimization part
    if loss_fn != None:
        if loss_fn.opt_start_step <= (scheduler.step_index*1.0 / len(scheduler.timesteps)):
            pred_x0_decode = vae.decode(pred_x0 / vae.config.scaling_factor, return_dict=False)[0].clamp(-1.0, 1.0).detach()
            with torch.enable_grad():
                pred_x0_decode = image_optimize(source_image, pred_x0_decode, loss_fn)
            pred_x0_decode = pred_x0_decode.detach().clamp(-1.0, 1.0)
            encoded = vae.encode(pred_x0_decode, return_dict=False)[0]
            pred_x0 = vae.config.scaling_factor * encoded.sample()
...
@torch.enable_grad()
def image_optimize(source, target, loss_fn):
    org_dtype = target.dtype
    source = source.to(dtype=torch.float32)
    target = target.to(dtype=torch.float32)
    target.requires_grad_(True)
    optimizer = SGD(params=[target], lr=1, momentum=0.9)
    for i in range(loss_fn.opt_iter):
        optimizer.zero_grad()
        loss = loss_fn(source, target)
        loss.backward()
        with torch.no_grad():
            # Replace NaN gradients with 0
            nan_mask = torch.isnan(target.grad)
            target.grad[nan_mask] = 0.0
        optimizer.step()

    target = target.to(dtype=org_dtype).detach()
    return target
...
class EditPipeline(DiffusionPipeline, TextualInversionLoaderMixin, StableDiffusionLoraLoaderMixin):
    ...
    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, denoise_model, generator=None):
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list):
                init_latents = [
                    self.vae.encode(image[i : i + 1], return_dict=False)[0].sample(generator[i])
                    for i in range(batch_size)
                ]
                init_latents = torch.cat(init_latents, dim=0)
            else:
                encoded = self.vae.encode(image, return_dict=False)[0]
                init_latents = encoded.sample(generator)
    ...
    def __call__(
        ...
        loss_fn: Optional[Callable[[torch.Tensor, torch.Tensor], float]] = None,
    ):
        # print(self.unet)
        # 1. Check inputs
        ...
        if not output_type == "latent":
            image_out = self.vae.decode(pred_x0 / self.vae.config.scaling_factor, return_dict=False)[0].clamp(-1.0, 1.0)
            if loss_fn is not None:
                if loss_fn.post_opt:
                    image_in = image.to(device=device, dtype=image_out.dtype)
                    image_out_gd = image_out.clone().detach()

                    with torch.enable_grad():
                        image_out_gd = image_optimize(image_in, image_out_gd, loss_fn)

                    image_out = image_out_gd.detach().clamp(-1.0, 1.0)
            image_out, has_nsfw_concept = self.run_safety_checker(image_out, device, prompt_embeds.dtype)
        else:
            image_out = pred_x0
            has_nsfw_concept = None
